...
def main():
    ...
    experiment_name = "threedfront_objfeat_vqvae"
    parser.add_argument(
        "--config_file", type=str,
        default=f"configs/{experiment_name}.yaml",
    ...
    # = select the device for run CUDA
    if torch.cuda.is_available():
    ...
    if not os.path.exists(args.output_dir):
        ...
        print(f"..>> Output directory created _ {args.output_dir}")
    else:
        print(f"..>> Output directory already exists _ {args.output_dir}")
    ...
    if args.tag is None:
        ...
        print(
            f"..>> Tag not specified"
            f"     using default tag: {tag}"
        )
    else:
        tag = args.tag
        print(f"..>> Tag specified: {tag}")
    ...
    if os.getenv("PATH_TO_OBJFEATS_TRAIN") and os.getenv("PATH_TO_OBJFEATS_VAL"):
        ...
        train_objects_wNone = pickle.load(open(os.getenv("PATH_TO_OBJFEATS_TRAIN"), "rb"))
        print(f"Load pickled validation 3D-FRONT object features from {os.getenv('PATH_TO_OBJFEATS_VAL')}")
        val_objects_wNone = pickle.load(open(os.getenv("PATH_TO_OBJFEATS_VAL"), "rb"))

        # + = select not None values
        train_objects = [v for v in train_objects_wNone if eval(f"v.{config['model']['objfeat_type']}_features") is not None]  #+ : remove None values
        val_objects = [v for v in val_objects_wNone if eval(f"v.{config['model']['objfeat_type']}_features") is not None]  #+ : remove None values

    else:
        
        scenes = parse_threed_front_scenes( # Load all scenes in 3D-FRONT
        ...
        ROOM_TYPES = [  # Collect objects used in three types of rooms
            "bedroom", 
            "bedroom", 
            "diningroom", 
            "livingroom"
        ]  # two "bedroom" for implementation convenience
        ...
        for scene_train_dataset in scene_train_datasets:
            for scene in scene_train_dataset:
                for obj in scene.bboxes:
                    train_objects_org[obj.model_jid] = obj
        train_objects_wNone = [vi for vi in train_objects_org.values()]
        # ... this contains None values
        train_objects = [v for v in train_objects_wNone if eval(f"v.{config['model']['objfeat_type']}_features") is not None]  #+ : remove None values

        # Save the training object features
        with open(
        ...
        for scene_val_dataset in scene_val_datasets:
            for scene in scene_val_dataset:
                for obj in scene.bboxes:
                    val_objects_org[obj.model_jid] = obj
        val_objects_wNone = [vi for vi in val_objects_org.values()]
        # ... this contains None values
        val_objects = [v for v in val_objects_wNone if eval(f"v.{config['model']['objfeat_type']}_features") is not None]  #+ : remove None values
        
        # Save the validation object features
        with open(
    ...
    all_objfeats_wNone = [[k, v] for k, v in all_objfeats_org.items()]  # unique object ids and features
    # ... this contains None values
    all_objfeats = [v for v in all_objfeats_wNone if v[1] is not None]  #+ : remove None values
