...
def export_scene(
    ...
    for i, m in enumerate(trimesh_meshes):
        ...
        renamed_tex = (mtl_names[i]+tex_ext).encode("ascii")
        orig_tex_name = os.path.basename(tex_key).encode("ascii")
        with open(path_to_mtl_file, "wb") as f:
            # Ensure the texture reference inside the .mtl is renamed to our generated texture file
            mtl_bytes = tex_out[mtl_key]
            mtl_bytes = mtl_bytes.replace(b"material_0.png", renamed_tex)
            mtl_bytes = mtl_bytes.replace(b"material_0.jpeg", renamed_tex)
            mtl_bytes = mtl_bytes.replace(orig_tex_name, renamed_tex)
            f.write(mtl_bytes)
