...
def main():
    ...
    # train_objfeats = [
    all_train_objfeats_wNone = [
        eval(f"obj.{config['model']['objfeat_type']}_features")
        for obj in train_objects
    ]
    # ... this contains None values
    train_objfeats = [v for v in all_train_objfeats_wNone if v is not None]  #+ : remove None values

    train_objfeats_stack = np.stack(train_objfeats, axis=0)
    kwargs = {
        "objfeat_min": train_objfeats_stack.min(),
        "objfeat_max": train_objfeats_stack.max()
    }
