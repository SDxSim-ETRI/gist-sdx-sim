...
class ThreedFutureFeatureDataset(ThreedFutureDataset):
    def __init__(self, objects, objfeat_type: str):
        ...
        feat_attr = f"{self.objfeat_type}_features"
        self._valid_indices = [
            idx for idx, obj in enumerate(self.objects)
            if getattr(obj, feat_attr, None) is not None
        ]

        if not self._valid_indices:
            raise ValueError(
                f"No objects contain '{feat_attr}' features; unable to construct dataset."
            )

        invalid_count = len(self.objects) - len(self._valid_indices)
        if invalid_count:
            warnings.warn(
                f"Skipping {invalid_count} object(s) without '{feat_attr}' features during access.",
                RuntimeWarning,
                stacklevel=2,
            )
    ...
    def __getitem__(self, idx):
        ...
        # eval(f"self.objects[32].{self.objfeat_type}_features") ... is None 
